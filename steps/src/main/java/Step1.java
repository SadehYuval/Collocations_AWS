...
public class Step1 {
	...
	public static class MapperClass extends Mapper<LongWritable, Text, Text, Text> {
		...
		String[] banned_eng = {"a", "about", "above", "across", "after", "afterwards", "again", "against", "all", "almost", "alone",
		...
		public void map(LongWritable key, Text value, Context context) {
			...
			try {
				...
				if (bigram.length == 2 && bigram[0].length()>1 && bigram[1].length()>1) {
					...
					// 	}
					// }
					if (banned.contains(word1.toLowerCase()) || banned.contains(word2.toLowerCase()))
						flag = false;
					if (flag) {
